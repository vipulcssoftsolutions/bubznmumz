...
?><div class='wpc-condition-group-wrap'>
	...
	<div class='wpc-condition-group clearfix' data-group='<?php echo absint( $condition_group ); ?>'>
		...
		<p class='match-text'><?php _e( 'Products that match all of the following rules will be added to the package', 'advanced-shipping-packages-for-woocommerce' ); ?></p>
		...
		<div class='wpc-conditions-list'><?php
			...
			if ( ! empty( $conditions ) ) :
				...
				foreach ( $conditions as $condition_id => $condition ) :
					$wp_condition = new ASPWC_Product_Condition( $condition_id, $condition_group, $condition['condition'], $condition['operator'], $condition['value'] );
					$wp_condition->output_condition_row();
				endforeach;

			else :

				$wp_condition = new ASPWC_Product_Condition( null, $condition_group );
				$wp_condition->output_condition_row();
		...
		<div class="wpc-condition-template hidden" style="display: none;"><?php
			$wp_condition = new ASPWC_Product_Condition( '9999', $condition_group );
			$wp_condition->output_condition_row();
		?></div>
		<a style="margin-top: 0.5em; margin-right: 63px;" class='wpc-condition-add wpc-add button alignright' href='javascript:void(0);'><?php _e( 'Add condition', 'advanced-shipping-packages-for-woocommerce' ); ?></a>
